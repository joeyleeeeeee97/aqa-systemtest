...
public class ServerlessTest implements StfPluginInterface {
    FileRef springPetclinicJar;

    @Override
    public void help(HelpTextGenerator help) throws Exception {
        help.outputSection("ServerlessTest");
        help.outputText("Test spring petclinic using Serverless");
    }
...
//                        .addProjectToClasspath("openjdk.test.serverless")
...
//        test.doKillProcesses("stop r1", defaultRunner);
        ...
        StfProcess ServerlessRunner = test.doRunBackgroundProcess("Serverless Run", "r3", ECHO_ON, ExpectedOutcome.neverCompletes(),
                test.createJavaProcessDefinition()
        ...
        test.doSleep("sleep", "10");
        test.doFindFileMatches("Assert Started in 10 seconds", ServerlessRunner.getStdoutFileRef(), "Started PetClinicApplication");
        test.doKillProcesses("stop r3", ServerlessRunner);
    }
